...
func startHTTPServer(ln net.Listener, port int, defaultOutputDir string, service core.DownloadService) {
	...
	mux.HandleFunc("/download", DownloadHandler(defaultOutputDir, service))
	mux.HandleFunc("/pause", PauseHandler(service))
	mux.HandleFunc("/pause-all", PauseAllHandler(service))
	mux.HandleFunc("/resume", ResumeHandler(service))
	mux.HandleFunc("/delete", DeleteHandler(service))
	mux.HandleFunc("/list", ListHandler(service))
	mux.HandleFunc("/history", HistoryHandler(service))
